...
const PRESET_COLORS = [
  "#ffffff", "#f5f4f0", "#F0EDE8", "#FFF3CD", "#D4EDDA",
  "#D1ECF1", "#F8D7DA", "#E2D9F3", "#FCE4D6", "#C8A847",
  "#6c757d", "#343a40", "#2C3E50", "#1A1A2E", "#0a0a0a",
];
...
const DEFAULT_NODES: CanvasNode[] = [
  { id: 0, x: 200, y: 180, w: 200, h: 90, title: "Project Idea", body: "Capture your thoughts here", type: "block", color: "#ffffff", fontSize: 13 },
  { id: 1, x: 500, y: 150, w: 200, h: 90, title: "Concept", body: "Connect your ideas", type: "block", color: "#ffffff", fontSize: 13 },
  { id: 2, x: 420, y: 320, w: 200, h: 70, title: "Next Steps", body: "", type: "block", color: "#ffffff", fontSize: 13 },
];
...
function ColorPickerWindow({
  ...
  const [hsv, setHsv] = useState<[number, number, number]>(() => hexToHsv(picker.color));
  ...
  useEffect(() => { applyHsvRef.current = applyHsv; }, [applyHsv]);
  ...
  return (
    <div
      ...
      style={{
        ...
        top: Math.max(8, Math.min(pos.y, window.innerHeight - pickerH - 340 - 8)),
        width: W,
    ...
    >
      <div
        onMouseDown={(e) => {
          e.preventDefault();
          isDraggingWindow.current = { ox: e.clientX - pos.x, oy: e.clientY - pos.y };
        }}
        style={{
      ...
      >
        <span style={{ fontSize: 12, fontWeight: 500, color: "#555", letterSpacing: "-0.1px" }}>
          Color
        </span>
      ...
      <div style={{ padding: "14px 16px 16px" }}>
        <div style={{
          height: 44, borderRadius: 10, background: currentHex,
          border: "0.5px solid rgba(0,0,0,0.1)", marginBottom: 12,
          boxShadow: "inset 0 1px 4px rgba(0,0,0,0.07)", transition: "background 0.05s",
        }} />
        ...
        <div
          ...
          style={{
            position: "relative", width: "100%", height: pickerH,
            borderRadius: 10, background: pureHueHex, cursor: "crosshair",
            marginBottom: 10, border: "0.5px solid rgba(0,0,0,0.08)",
            overflow: "hidden", flexShrink: 0, transition: "height 0.2s ease",
          }}
        >
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, #fff, transparent)" }} />
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, transparent, #000)" }} />
          <div style={{
            position: "absolute", left: `${hsv[1]}%`, top: `${100 - hsv[2]}%`,
        ...
        <div style={{ position: "relative", height: 18, marginBottom: 14 }}>
          <div style={{
            ...
            border: "0.5px solid rgba(0,0,0,0.08)", pointerEvents: "none",
          }} />
          <div style={{
            position: "absolute", left: `calc(${(hsv[0] / 360) * 100}% - 8px)`, top: "50%",
            transform: "translateY(-50%)", width: 16, height: 16, borderRadius: "50%",
            background: pureHueHex, border: "2px solid #fff",
            boxShadow: "0 0 0 1px rgba(0,0,0,0.18), 0 2px 4px rgba(0,0,0,0.22)",
            pointerEvents: "none",
          }} />
          <input
            type="range" min={0} max={360} value={hsv[0]}
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 14, alignItems: "flex-start" }}>
          <div style={{
            flex: 2, display: "flex", alignItems: "center", gap: 6,
            background: "rgba(0,0,0,0.04)", borderRadius: 8, padding: "6px 9px",
            border: "0.5px solid rgba(0,0,0,0.08)",
          }}>
            <div style={{ width: 14, height: 14, borderRadius: 4, background: currentHex, border: "0.5px solid rgba(0,0,0,0.12)", flexShrink: 0 }} />
            <input
              value={hexInput}
              onChange={(e) => { setHexInput(e.target.value); if (isValidHex(e.target.value)) applyHex(e.target.value); }}
              onBlur={() => { if (!isValidHex(hexInput)) setHexInput(currentHex); }}
              style={{ flex: 1, border: "none", background: "transparent", fontSize: 11, fontFamily: "monospace", color: "#333", outline: "none", minWidth: 0, letterSpacing: "0.3px" }}
            />
          </div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(n) && n >= 0 && n <= 255) {
                    const rgb: [number, number, number] = [parseInt(rgbInput[0]) || 0, parseInt(rgbInput[1]) || 0, parseInt(rgbInput[2]) || 0];
                    rgb[i] = n;
                ...
                style={{ width: "100%", textAlign: "center", border: "0.5px solid rgba(0,0,0,0.1)", background: "rgba(0,0,0,0.03)", borderRadius: 7, padding: "5px 2px", fontSize: 11, color: "#333", outline: "none", fontFamily: "monospace" }}
              />
              <span style={{ fontSize: 9, color: "#bbb", letterSpacing: "0.4px", fontWeight: 500 }}>{label}</span>
            </div>
          ))}
        </div>

        <div>
          <div style={{ fontSize: 10, color: "#bbb", letterSpacing: "0.5px", textTransform: "uppercase", marginBottom: 7 }}>
            Quick Picks
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 5 }}>
            {PRESET_COLORS.map((c) => {
              ...
              return (
                <div
                  key={c} onClick={() => applyHex(c)}
                  style={{
                    height: 24, borderRadius: 6, background: c,
                    border: active ? `2px solid ${ACCENT}` : "1px solid rgba(0,0,0,0.1)",
...
export default function Canvas() {
  ...
  const addNode = useCallback(
    (cx: number, cy: number, type: NodeType) => {
      ...
      const newNode: CanvasNode = {
        ...
        title: isText ? "" : type === "circle" ? "Circle" : type === "diamond" ? "Diamond" : type === "rounded" ? "Area" : "New Block",
        body: "", type,
  ...
  const onImageFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      setNodes((prev) => {
        ...
        if (idCounter <= maxId) idCounter = maxId + 1;
        return [...prev, { id: idCounter++, x: pos.cx - 150, y: pos.cy - 100, w: 300, h: 200, title: "Image", body: "", type: "image", color: "#ffffff", imageUrl }];
      });
  ...
  const openColorPicker = useCallback(
    (nodeId: number, currentColor: string, screenX: number, screenY: number) => {
      setColorPicker({ nodeId, x: screenX - 280, y: screenY - 20, color: currentColor });
      setContextMenu(null);
    },
    [],
  );
  ...
  useEffect(() => {
    if (!hydrated) return;
    try { localStorage.setItem(LS_NODES, JSON.stringify(nodes)); } catch {}
  }, [nodes, hydrated]);
  ...
  const onCanvasContextMenu = useCallback(
    (e: React.MouseEvent) => {
      ...
      setContextMenu({ kind: "canvas", x: e.clientX, y: e.clientY, cx: pos.x, cy: pos.y });
    },
    [toCanvas],
  );
  ...
  const onMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (dragging.current && canvasRef.current) {
        ...
        setNodes((prev) => prev.map((n) => (n.id === id ? { ...n, x: mx - ox, y: my - oy } : n)));
      }
      if (resizing.current) {
        ...
        setNodes((prev) =>
          prev.map((n) => n.id === id ? { ...n, w: Math.max(80, startW + dx), h: Math.max(50, startH + dy) } : n),
        );
      }
  ...
  const onMouseUp = useCallback(() => {
    ...
    if (connectDrag) {
      ...
      if (targetId !== null && targetId !== connectDrag.fromId) {
        setConnections((prev) => {
          const dup = prev.some((c) => c.from === connectDrag.fromId && c.to === targetId);
          return dup ? prev : [...prev, { from: connectDrag.fromId, to: targetId }];
        });
      }
      setConnectDrag(null);
    }
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      const t = e.target as HTMLElement;
      if ((e.key === "Delete" || e.key === "Backspace") && !t.isContentEditable && selected !== null)
        deleteSelected();
      if (e.key === "Escape") {
        setConnectDrag(null);
        setContextMenu(null);
  ...
  return (
    <div
      style={{
        width: "100vw", height: "100vh", background: "#f5f4f0",
    ...
    >
      <input
        ...
        style={{ display: "none" }} onChange={onImageFileChange}
      />
        ...
        {connectDrag && (
          <>
            ...
            <div
              ...
              style={{ fontSize: 11, color: "#aaa", cursor: "pointer", padding: "2px 6px", borderRadius: 5, marginLeft: 2 }}
          ...
          <svg style={{ position: "absolute", left: -5000, top: -5000, width: 10000, height: 10000, pointerEvents: "none", overflow: "visible" }}>
            ...
            {connections.map((c) => {
              ...
              return (
                <path
                  ...
                  stroke="#c8c6c0" strokeWidth={1.5 / zoom} fill="none" strokeLinecap="round"
                />
              );
            })}
            ...
            {connectDrag && (() => {
              ...
              return (
                <path
                  d={`M ${x1} ${y1} C ${cxm} ${y1}, ${cxm} ${y2}, ${x2} ${y2}`}
                  stroke={ACCENT} strokeWidth={2 / zoom} fill="none" strokeLinecap="round"
        ...
        >
          ...
          {(
            [
              ...
              { type: "rounded" as NodeType, label: "Area (rounded)", icon: "▢" },
      ...
      {/* ── Color Picker ── */}
      {colorPicker && (
        <ColorPickerWindow picker={colorPicker} onColorChange={onPickerColorChange} onClose={() => setColorPicker(null)} />
      )}
      ...
      >
        <button onClick={() => setZoom((z) => Math.max(0.2, parseFloat((z - 0.1).toFixed(2))))} style={{ border: "none", background: "none", fontSize: 18, cursor: "pointer", color: "#555", lineHeight: 1 }}>−</button>
        <span style={{ fontSize: 11, color: "#999", minWidth: 38, textAlign: "center" }}>{Math.round(zoom * 100)}%</span>
        <button onClick={() => setZoom((z) => Math.min(3, parseFloat((z + 0.1).toFixed(2))))} style={{ border: "none", background: "none", fontSize: 18, cursor: "pointer", color: "#555", lineHeight: 1 }}>+</button>
        <div style={{ width: "0.5px", height: 16, background: "rgba(0,0,0,0.1)" }} />
        <button onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }} style={{ border: "none", background: "none", fontSize: 11, cursor: "pointer", color: "#aaa" }}>Reset</button>
      </div>
